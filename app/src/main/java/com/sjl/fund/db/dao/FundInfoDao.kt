...
interface FundInfoDao {
    ...
    @Query("SELECT * FROM fund_info WHERE fundType = :fundType order by  sortId desc,createTime desc")
    fun listFundInfosByType(fundType: Int): MutableList<FundInfo>?


    @Query("SELECT fundcode FROM fund_info order by sortId desc,createTime desc")
